...
async def test_client_state_list():
    ...
    try:
        client = MyFoxApiShutterClient(myfox_info)
        client.nb_retry = 1
...
@pytest.mark.asyncio
async def test_client_set_favorite():
    _LOGGER.info("**** Debut ****")
    myfox_info = MyFoxMockCache.getMyFoxEntryDataFromCache()
    try:
        client = MyFoxApiShutterClient(myfox_info)
        ...
        client.delay_between_retry = 1
        # get list
        results = await client.setFavorite(2468)
        _LOGGER.info("setFavorite(1):" + str(results))
        assert results
...
@pytest.mark.asyncio
async def test_client_set_open():
    _LOGGER.info("**** Debut ****")
    myfox_info = MyFoxMockCache.getMyFoxEntryDataFromCache()
    try:
        client = MyFoxApiShutterClient(myfox_info)
        ...
        client.delay_between_retry = 1
        # get list
        results = await client.setOpen(2468)
        _LOGGER.info("setOpen(1):" + str(results))
        assert results
...
@pytest.mark.asyncio
async def test_client_set_close():
    _LOGGER.info("**** Debut ****")
    myfox_info = MyFoxMockCache.getMyFoxEntryDataFromCache()
    try:
        client = MyFoxApiShutterClient(myfox_info)
        ...
        client.delay_between_retry = 1
        # get list
        results = await client.setClose(2468)
        _LOGGER.info("setClose(1):" + str(results))
        assert results
