...
class MyFoxApThermoClient(MyFoxApiClient) :
    ...
    async def getList(self) -> list:
        """ Get security site """
        try:
            response = await self.callMyFoxApiGet(MYFOX_DEVICE_HEATER_THERMO_LIST % (self.myfox_info.siteId))
            print(str(response))
    ...
    async def setAuto(self, device:MyFoxHeater) -> list:
        """ Get security site """
        try:
            response = await self.callMyFoxApiPost(MYFOX_DEVICE_HEATER_THERMO_SET_AUTO % (self.myfox_info.siteId, device.deviceId))
    ...
    async def setAway(self, device:MyFoxHeater) -> list:
        """ Get security site """
        try:
            response = await self.callMyFoxApiPost(MYFOX_DEVICE_HEATER_THERMO_SET_AWAY % (self.myfox_info.siteId, device.deviceId))
    ...
    async def setBoost(self, device:MyFoxHeater) -> list:
        """ Get security site """
        try:
            response = await self.callMyFoxApiPost(MYFOX_DEVICE_HEATER_THERMO_SET_BOOST % (self.myfox_info.siteId, device.deviceId))
    ...
    async def setOff(self, device:MyFoxHeater) -> list:
        """ Get security site """
        try:
            response = await self.callMyFoxApiPost(MYFOX_DEVICE_HEATER_THERMO_SET_OFF % (self.myfox_info.siteId, device.deviceId))
