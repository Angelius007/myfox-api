...
class MyFoxApiTemperatureClient(MyFoxApiClient) :
    ...
    async def getTemperatureList(self) -> list:
        """ Get security site """
        try:
            response = await self.callMyFoxApiGet(MYFOX_DEVICE_TEMPERATURE_LIST % (self.myfox_info.siteId))
    ...
    async def getTemperature(self, device:MyFoxTemperatureSensor) -> list:
        """ Get security site """
        try:
            response = await self.callMyFoxApiGet(MYFOX_DEVICE_TEMPERATURE_GET % (self.myfox_info.siteId, device.deviceId))
            items = response["payload"]["items"]
